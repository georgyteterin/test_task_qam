...
data_cpp = readmatrix("results/qam_16_results.txt");
numBits = data_cpp(1);
...
for k=1:length(EbN0)
    ...
    txSig = qammod(bits, 16, "InputType","bit");
    rxSig = awgn(txSig, EbN0(k));
    out_bits = qamdemod(rxSig, 16, "OutputType", "bit");
...
title("Помехоустойчивость 16-QAM");
xlabel("EbN0");
ylabel("BER");
...
grid on
legend("Реализация в C++", "Реализация в MATLAB")
title("Зависимость BER от дисперсии шума 16-QAM");
